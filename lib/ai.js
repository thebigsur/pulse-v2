...
export async function classifyPosts(posts, categories) {
  ...
  for (let i = 0; i < posts.length; i += 10) {
    ...
    const postList = batch.map((p, idx) => 
      `[${idx}] ${(p.post_text || '').slice(0, 200)}`
    ).join('\n\n');

    const prompt = `Classify each post into exactly ONE category from the list below. If no category fits well, use "General".
...
Rules:
- Each post gets exactly one category from the list above, or "General"
- Be consistent: similar posts should get the same category
- Match based on the primary topic/theme of the post
- If a post could fit multiple categories, choose the most specific one
