...
export async function generateDraft(sourcePost, advisorProfile, postHistory, voiceSamples, contentPrefs) {
  ...
  const last14 = postHistory.filter(p => p.posted_at && new Date(p.posted_at) >= fourteenDaysAgo);
...
1. NEVER REPEAT: Topics posted in the last 14 days are OFF LIMITS for the same angle.
   Off-limits topics: ${last14Topics.join(', ') || 'none'}
   You CAN revisit a topic from 14+ days ago with a COMPLETELY different angle.
