...
export async function generateDraft(sourcePost, advisorProfile, postHistory, voiceSamples, contentPrefs) {
  ...
  // Normalize topic tags to readable names for the LLM prompt
  ...
  // Also pull any topics already in the draft queue (passed via postHistory convention)
  // postHistory entries with source='draft_queue' signal queued topics
  const queuedDraftTopics = postHistory
    .filter(p => p.source === 'draft_queue')
    .flatMap(p => p.topic_tags || [])
    .map(normalizeTopicForPrompt);

  const allOffLimitsTopics = [...new Set([...last14Topics, ...queuedDraftTopics])];
...
1. NEVER REPEAT: Topics posted in the last 14 days are OFF LIMITS for the same angle.
   Off-limits topics (use these EXACT readable names to recognize the topic — do not draft on any of these): ${allOffLimitsTopics.join(', ') || 'none yet'}
   
   IMPORTANT: If the source post is primarily about an off-limits topic, set "skip_reason" instead of generating a draft. Do NOT try to rework an RSU post into a barely-different RSU post just because the angle is slightly different. If the core subject (RSUs, Roth conversions, etc.) appears in the off-limits list, SKIP it.

2. NATURAL FLOW: The new post should feel like a natural next step in the advisor's content. Look at what they've covered recently and find a topic or angle that complements it without repeating.
...
${advisorProfile.content_filters ? `\nCONTENT FILTERS (advisor has specifically flagged these — if the source post matches any, set skip_reason):\n${advisorProfile.content_filters}` : ''}
...
- If this source topic was already covered in the last 14 days, set "skip_reason" instead of generating a draft
...
Respond ONLY with valid JSON, no markdown:
{
  ...
  try {
    ...
    // If AI decided to skip due to topic overlap
    if (parsed.skip_reason) {
      console.log(`[Draft] Skipped: ${parsed.skip_reason}`);
      return null;
    }
    return parsed;
  } catch (err) {
