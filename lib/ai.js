...
export async function classifyPosts(posts, categories) {
  ...
  // Financial keywords — if a post contains these, it's NOT personal/family
  const FINANCIAL_KEYWORDS = [
    'wealth', 'estate', 'tax', 'retire', 'invest', 'stock', 'rsu', 'iso', 'nso',
    'equity', 'portfolio', 'capital gain', '401k', '401(k)', 'ira', 'roth',
    'inheritance', 'beneficiary', 'trust', 'will ', 'probate', 'fiduciary',
    'financial plan', 'net worth', 'compound', 'dividend', 'market', 'valuation',
    's&p', 'earnings', 'interest rate', 'inflation', 'mortgage', 'home price',
    'real estate', 'debt', 'salary', 'compensation', 'bonus', 'vesting',
    'generation', 'transfer', 'legacy', 'asset', 'saving', 'budget',
    'million', 'billion', 'trillion', '$', 'percent', '%',
  ];

  const hasFinancialContent = (text) => {
    const lower = (text || '').toLowerCase();
    return FINANCIAL_KEYWORDS.some(kw => lower.includes(kw));
  };

  // Find the best financial fallback category
  const findFinancialFallback = (text, cats) => {
    const lower = (text || '').toLowerCase();
    // Priority matching
    if (/wealth\s*transfer|inheritance|estate|generation|legacy|beneficiar/.test(lower)) {
      const match = cats.find(c => /estate|wealth|legacy/i.test(c));
      if (match) return match;
    }
    if (/market|valuation|s&p|economy|home price|real estate|interest rate|inflation/.test(lower)) {
      const match = cats.find(c => /market/i.test(c));
      if (match) return match;
    }
    if (/rsu|iso|nso|equity|vesting|stock option|concentrated/.test(lower)) {
      const match = cats.find(c => /equity/i.test(c));
      if (match) return match;
    }
    if (/tax|capital gain|amt|harvest/.test(lower)) {
      const match = cats.find(c => /tax/i.test(c));
      if (match) return match;
    }
    if (/401k|401\(k\)|pension|employer.*retire/.test(lower)) {
      const match = cats.find(c => /employer.*retire|retirement/i.test(c));
      if (match) return match;
    }
    if (/roth|ira|sep|solo 401/.test(lower)) {
      const match = cats.find(c => /non.*employer.*retire|ira/i.test(c));
      if (match) return match;
    }
    // Generic financial fallback
    const genFinance = cats.find(c => /general.*financial|financial.*plan/i.test(c));
    if (genFinance) return genFinance;
    // Last resort: first category that isn't personal/family/general
    return cats.find(c => !/personal|family|general/i.test(c)) || cats[0];
  };
...
EXPLICIT EXAMPLES OF WHAT IS NOT PERSONAL:
- "A massive amount of wealth is expected to pass from older generations to their children" → This is about WEALTH TRANSFER/ESTATE PLANNING, NOT personal/family
- "Buying a home is harder than ever for young families" → This is MARKET COMMENTARY, NOT personal/family
- "My parents never talked about money with me" → If it leads to financial planning advice, it's FINANCIAL
- "I helped a client's family save $50K in taxes" → This is TAX STRATEGY, NOT personal/family

ONLY classify as "Personal or family" for posts like:
- "Had an amazing weekend with the kids at Disneyland"
- "Happy 10th anniversary to my wife"
- "My son scored his first goal in soccer today"
...
[{"index": 0, "category": "<exact category name from list>"}, ...]`;
    ...
    try {
      ...
      for (const item of parsed) {
        ...
        if (post) {
          let category = categories.includes(item.category) ? item.category : (item.category === 'General' ? 'General' : categories[0]);
          
          // SAFETY NET: If classified as personal/family but contains financial keywords, reclassify
          if (/personal|family/i.test(category) && hasFinancialContent(post.post_text)) {
            const betterCat = findFinancialFallback(post.post_text, categories);
            console.log(`Reclassified "${(post.post_text || '').slice(0, 60)}..." from "${category}" to "${betterCat}" (financial keyword detected)`);
            category = betterCat;
          }
