...
export async function scrapeLinkedInContent(keywords = []) {
  ...
  for (let i = 0; i < keywords.length; i += BATCH_SIZE) {
    ...
    try {
      console.log(`[Content Feed] Starting LinkedIn scrape for batch: ${batch.map(k => `"${k}"`).join(', ')}`);
      ...
      console.log(`[Content Feed] Got ${items.length} LinkedIn items for batch [${batch.join(', ')}]`);
      ...
      console.log(`[Content Feed] Batch [${batch.join(', ')}]: ${items.length} raw, ${results.length} total kept`);
    } catch (err) {
      console.error(`LinkedIn scrape error for batch [${batch.join(', ')}]:`, err.message);
    }
  }
...
export async function scrapeLinkedInCommentFeed(keywords = []) {
  ...
  for (let i = 0; i < keywords.length; i += BATCH_SIZE) {
    const batch = keywords.slice(i, i + BATCH_SIZE);
    try {
      console.log(`[Comment Feed] Starting scrape for batch: ${batch.map(k => `"${k}"`).join(', ')}`);
      const run = await client.actor('harvestapi/linkedin-post-search').call({
        searchQueries: batch,
        maxPosts: 15,
        sortBy: 'relevance',
      ...
      console.log(`[Comment Feed] Run finished for batch [${batch.join(', ')}], status: ${run.status}, datasetId: ${run.defaultDatasetId}`);
      const { items } = await client.dataset(run.defaultDatasetId).listItems({ limit: 45 });
      console.log(`[Comment Feed] Got ${items.length} raw items for batch [${batch.join(', ')}]`);
      ...
      console.log(`[Comment Feed] Batch [${batch.join(', ')}]: ${items.length} raw, ${filtered} filtered, ${results.length} total kept`);
    } catch (err) {
      console.error(`LinkedIn comment feed scrape error for batch [${batch.join(', ')}]:`, err.message);
    }
  }
