    ...
    try {
      ...
      for (const item of items) {
        ...
        results.push({
          ...
          post_text: text,
          url: item.url || item.postUrl || (item.urn ? `https://www.linkedin.com/feed/update/${item.urn}` : (item.id ? `https://www.linkedin.com/feed/update/urn:li:activity:${item.id}` : '')),
  ...
  for (const keyword of keywords) {
    try {
      ...
      if (items.length > 0 && results.length === 0) {
        ...
        const first = items[0];
        console.log(`[Comment Feed] Sample — text: ${(first.text || '').substring(0, 50)}, title: ${(first.title || '').substring(0, 50)}, postText: ${(first.postText || '').substring(0, 50)}`);
        console.log(`[Comment Feed] Sample — author: ${JSON.stringify(first.author || first.authorName || 'none').substring(0, 100)}`);
        console.log(`[Comment Feed] Sample — engagement: ${JSON.stringify(first.engagement || 'none').substring(0, 100)}`);
        console.log(`[Comment Feed] Sample — url: ${first.url || 'none'}, postUrl: ${first.postUrl || 'none'}, urn: ${first.urn || 'none'}, id: ${first.id || 'none'}`);
      }
